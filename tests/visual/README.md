...
The five configured routes are the actual `PAGE_SPECS` titles and slugs in `components/carbon_ui.py`. Direct Streamlit routes are attempted first; the scripts fall back to the visible custom shell when a route does not render the expected title. The packaged custom component exposes stable selectors for the intentional interaction contract: `global-filter-open`, `global-filter-apply`, `global-filter-reset`, `sidebar-toggle`, and `sidebar-nav`. The objective check validates tablet sidebar collapse/reopen; filter apply/reset remain capability-detected but are not claimed as automated behavior until the component has deterministic state assertions.
